...
import { Search, Calendar, ChevronLeft, ChevronRight, RefreshCw, AlertCircle, Inbox, Image as ImageIcon } from 'lucide-react';
import HeroSection from '../components/ui/HeroSection';
import AnimateIn from '../components/ui/AnimateIn';

const BASE_URL = 'https://newhorizoncollegeofengineering.in/wp-json/wp/v2';
const NEWS_CATEGORY_ID = '11';
...
export default function NewsListPage() {
  ...
  // Re-fetch when perPage or order changes
  useEffect(() => {
    fetchPosts(1);
  ...
  return (
    <div className="min-h-screen bg-slate-50">
      <HeroSection
        ...
        badge="News"
        headingSmall="Stay in the loop"
        headingMain="Latest"
        headingGhost="News"
        description="Announcements, achievements and updates straight from New Horizon College of Engineering."
        button={{ label: 'Read Latest', onClick: () => document.getElementById('news-content')?.scrollIntoView({ behavior: 'smooth' }) }}
      />

      {/* ── Search & Filter Form ── */}
      <div id="news-content" className="sticky top-[80px] sm:top-[64px] z-30 bg-white border-b border-slate-100 py-4 scroll-mt-[140px] sm:scroll-mt-[124px] shadow-sm">
      ...
      <div className="container-wide py-10">
                ...
                <article className="group bg-white rounded-2xl border border-slate-200 overflow-hidden shadow-sm hover:shadow-md hover:border-blue-200 hover:-translate-y-1 transition-all duration-300 flex flex-col h-full">
                  ...
                  <div className="p-6 flex flex-col flex-1">
                    ...
                    <div className="mt-auto border-t border-slate-100 pt-4">
                      <Link to={`/news/${post.slug}`} className="inline-flex items-center gap-1.5 text-base font-semibold text-blue-600 hover:text-blue-800 transition-colors group/link">
        ...
        {!loading && !error && posts.length === 0 && (
          <div className="bg-white border border-slate-200 rounded-3xl p-12 flex flex-col items-center justify-center text-center">
            ...
            <p className="text-base text-slate-500">Try adjusting your search to find what you're looking for.</p>
          </div>
        )}
